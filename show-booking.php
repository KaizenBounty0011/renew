...
// Fetch show
$stmt = $pdo->prepare("SELECT * FROM entertainment_shows WHERE id = ? AND status = 'upcoming'");
$stmt->execute([$showId]);
$show = $stmt->fetch();
...
$pageTitle = 'Book Tickets - ' . $show['show_name'];
$errors = [];
$success = false;
$bookingRef = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $unitPrice = ($ticketType === 'vip') ? $show['vip_price'] : $show['ticket_price'];
    $calculatedTotal = $unitPrice * $ticketQuantity;

    // Check availability
    if ($ticketQuantity > $show['available_tickets']) {
        $errors[] = 'Sorry, only ' . $show['available_tickets'] . ' tickets are available.';
    }

    if (empty($errors)) {
        $bookingRef = generateBookingRef('RS');

        $stmt = $pdo->prepare("INSERT INTO show_bookings (show_id, customer_name, email, phone, ticket_type, ticket_quantity, total_amount, booking_reference) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([
            $show['id'],
            $customerName,
            $email,
        ...
        $pdo->prepare("UPDATE entertainment_shows SET available_tickets = available_tickets - ? WHERE id = ?")->execute([$ticketQuantity, $show['id']]);
...
?>
    ...
    <section class="page-banner" style="background: linear-gradient(135deg, #1a1a2e, #0a0a1a);">
        <div class="container">
            <h1>Book Tickets</h1>
            <p><?= sanitize($show['show_name']) ?></p>
            <div class="breadcrumb">
                <a href="<?= SITE_URL ?>">Home</a>
                <span>/</span>
                <a href="<?= SITE_URL ?>/entertainment.php">Entertainment</a>
                <span>/</span>
                <a href="<?= SITE_URL ?>/show-details.php?slug=<?= $show['slug'] ?>"><?= sanitize($show['show_name']) ?></a>
                <span>/</span>
    ...
    <section class="section">
        <div class="container">
            ...
            <div class="success-page">
                ...
                <p>Your tickets for <strong><?= sanitize($show['show_name']) ?></strong> have been reserved successfully.</p>
                ...
                <div style="display: flex; gap: 15px; justify-content: center; margin-top: 25px; flex-wrap: wrap;">
                    <a href="<?= SITE_URL ?>/entertainment.php" class="btn btn-entertainment">Browse More Shows <i class="fas fa-arrow-right"></i></a>
            ...
            <!-- Booking Form -->
            <div class="form-card">
                <h2><i class="fas fa-ticket-alt" style="color: var(--entertainment); margin-right: 10px;"></i> Book Your Tickets</h2>
                ...
                <div style="background: var(--light); padding: 20px; border-radius: var(--radius); margin-bottom: 25px; display: flex; gap: 20px; align-items: center; flex-wrap: wrap;">
                    <div style="flex: 1; min-width: 200px;">
                        <h3 style="font-size: 1.1rem; margin-bottom: 5px;"><?= sanitize($show['show_name']) ?></h3>
                        <p style="font-size: 0.85rem; color: var(--text-light);"><i class="far fa-calendar"></i> <?= date('M d, Y \a\t g:i A', strtotime($show['show_date'])) ?></p>
                        <p style="font-size: 0.85rem; color: var(--text-light);"><i class="fas fa-map-marker-alt"></i> <?= sanitize($show['venue']) ?>, <?= sanitize($show['location']) ?></p>
                    </div>
                ...
                <form method="POST" data-validate>
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            ...
                            <select id="ticket_type" name="ticket_type" class="form-control" required>
                                <option value="regular" data-price="<?= $show['ticket_price'] ?>" <?= (isset($ticketType) && $ticketType === 'regular') ? 'selected' : '' ?>>Regular - <?= formatPrice($show['ticket_price']) ?></option>
                                <option value="vip" data-price="<?= $show['vip_price'] ?>" <?= (isset($ticketType) && $ticketType === 'vip') ? 'selected' : '' ?>>VIP - <?= formatPrice($show['vip_price']) ?></option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="booking-summary">
                        ...
                        <div class="summary-row">
                            <span>Date</span>
                            <span><?= date('M d, Y', strtotime($show['show_date'])) ?></span>
                        </div>
                        <div class="summary-row">
                            <span>Venue</span>
                            <span><?= sanitize($show['venue']) ?></span>
                        </div>
                        <div class="summary-row summary-total">
                            <span>Total Amount</span>
                            <span id="total_display"><?= formatPrice($show['ticket_price']) ?></span>
                        </div>
                    </div>

                    <div class="text-center mt-30">
                        <button type="submit" class="btn btn-entertainment btn-lg" style="width: 100%;">
